...
class MEIService {
  Future <MEIData> fetchJSON() async {
    final response= await http.get(Uri.parse("http://10.81.83.155:8000/mei"));
  ...
  final response =
      await http.get(Uri.parse("http://10.81.83.155:8000/stock/$code"));
  ...
  Future<List<int>> fetchMEIHistory(String code) async {
    final response = await http.get(Uri.parse("http://10.81.83.155:8000/stock/history/$code"));
    final data = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> fetchMEItrend(String code) async {
    final response= await http.get(Uri.parse("http://10.81.83.155:8000/stock/historical_trend/$code"));
    final data=jsonDecode(response.body);
    return data;
  ...
  final response = await http.post(
    Uri.parse("http://10.81.83.155:8000/assistant_chat"),
    headers: {
