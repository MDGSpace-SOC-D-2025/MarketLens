...
class MEIService {
  ...
  Future<List<int>> fetchMEIHistory(String code) async {
    final response = await http.get(Uri.parse("http://192.168.1.5:8000/stock/history/$code"));
    final data = jsonDecode(response.body);
  ...
  Future<Map<String, dynamic>> fetchMEItrend(String code) async {
    final response= await http.get(Uri.parse("http://192.168.1.5:8000/stock/historical_trend/$code"));
    final data=jsonDecode(response.body);
    return data;
